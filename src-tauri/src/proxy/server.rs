...
async fn admin_list_accounts(
    ...
    let account_responses: Vec<AccountResponse> = accounts
        ...
        .map(|acc| {
            ...
            let rate_limit_reset_seconds =
                state.token_manager.get_rate_limit_reset_seconds(&acc.id);
...
async fn admin_save_config(
    ...
    crate::proxy::update_cache_management_config(new_config.proxy.cache_management.clone());
