...
pub fn init_db() -> Result<(), String> {
    ...
    let _ = conn.execute("ALTER TABLE request_logs ADD COLUMN cache_read_input_tokens INTEGER", []);
    let _ = conn.execute("ALTER TABLE request_logs ADD COLUMN cache_creation_input_tokens INTEGER", []);
...
fn proxy_log_from_row(row: &Row<'_>) -> rusqlite::Result<ProxyRequestLog> {
    Ok(ProxyRequestLog {
        id: row.get(0)?,
        timestamp: row.get(1)?,
        method: row.get(2)?,
        ...
        cache_read_input_tokens: row.get(12).unwrap_or(None),
        cache_creation_input_tokens: row.get(13).unwrap_or(None),
...
pub fn save_log(log: &ProxyRequestLog) -> Result<(), String> {
    ...
    conn.execute(
        "INSERT INTO request_logs (id, timestamp, method, url, status, duration, model, error, request_body, response_body, input_tokens, output_tokens, cache_read_input_tokens, cache_creation_input_tokens, account_email, mapped_model, protocol, client_ip, username)
